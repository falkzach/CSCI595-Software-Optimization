...
constexpr unsigned int NUCLEOTITES_PER_BLOCK = sizeof(unsigned long long) * 8 / 2;

void bit_pack(unsigned long long * & result, unsigned long & num_blocks,
	...
	result = (unsigned long long*)calloc(num_blocks, sizeof(unsigned long long));
...
unsigned long long* packed_complement(const unsigned long long*packed_genome,
	unsigned long num_blocks) {
	unsigned long long * result = (unsigned long long*)calloc(num_blocks, sizeof(unsigned long long));
...
int main() {
	...
	unsigned long long*comp = packed_complement(packed, num_blocks);
	...
	for (unsigned int i=0; i<2; ++i) {
		std::cout << std::bitset<8*sizeof(unsigned long long)>(packed[i]);
	}
	std::cout << "..." << std::endl;

	for (unsigned int i=0; i<2; ++i) {
		std::cout << std::bitset<8*sizeof(unsigned long long)>(comp[i]);
	}
	std::cout << "..." << std::endl;
