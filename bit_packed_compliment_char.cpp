...
constexpr unsigned int NUCLEOTITES_PER_BLOCK = sizeof(unsigned char) * 8 / 2;

void bit_pack(unsigned char * & result, unsigned long & num_blocks,
	...
	result = (unsigned char*)calloc(num_blocks, sizeof(unsigned char));
...
unsigned char* packed_complement(const unsigned char*packed_genome,
	unsigned long num_blocks) {
	unsigned char * result = (unsigned char*)calloc(num_blocks, sizeof(unsigned char));
...
int main() {
	...
	unsigned char*comp = packed_complement(packed, num_blocks);
	...
	for (unsigned int i=0; i<2; ++i) {
		std::cout << std::bitset<8*sizeof(unsigned char)>(packed[i]);
	}
	std::cout << "..." << std::endl;

	for (unsigned int i=0; i<2; ++i) {
		std::cout << std::bitset<8*sizeof(unsigned char)>(comp[i]);
	}
	std::cout << "..." << std::endl;
