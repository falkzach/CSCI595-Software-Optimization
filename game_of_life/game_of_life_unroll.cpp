...
bool advance_single(bool*board, const unsigned int C, int r, int c) {
	const unsigned char living_neighbors =  (board[(r-1) * (C+ (padding * 2)) + (c-1)])
										 +	(board[(r-1) * (C+ (padding * 2)) + (c)])
										 +	(board[(r-1) * (C+ (padding * 2)) + (c+1)])

										 +	(board[(r) * (C+ (padding * 2)) + (c-1)])
										 +	(board[(r) * (C+ (padding * 2)) + (c)])
										 +	(board[(r) * (C+ (padding * 2)) + (c+1)])
